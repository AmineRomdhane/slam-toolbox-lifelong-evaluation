...
for i in range(1,args.runs+1):
    ...
    try:
        with (root/f'simulator_{i}.log').open('w') as sl,(root/f'ground_truth_{i}.log').open('w') as gl,(root/f'run_{i}.log').open('w') as cl:
            ...
            gt=start(['/home/roam5170/slam_testing/install/slam_eval_ground_truth/lib/slam_eval_ground_truth/ground_truth_node'],gl)
            time.sleep(4)
            out=root/f'run_{i}'
            ctl=start(['/home/roam5170/slam_testing/install/slam_eval_route_controller/lib/slam_eval_route_controller/route_controller','--ros-args','-p','use_sim_time:=true','-p',f'output_dir:={out}'],cl)
