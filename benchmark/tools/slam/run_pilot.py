...
WS=Path(__file__).resolve().parents[3];HERE=Path(__file__).resolve().parent
...
OUT=WS/'results/static/world_v0/r1'/args.run_id;CONFIG=WS/'benchmark/config/mapper_params_online_async.yaml'
...
manifest=json.loads((WS/'benchmark/manifests/configuration_manifest.json').read_text());assert sha(CONFIG)==manifest['configuration_sha256']
